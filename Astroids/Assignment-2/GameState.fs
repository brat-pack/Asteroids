...
and SpaceShip = {
    ...
    member this.Update(dt: GameTime, gameState) =
        if (this.collidesWithAsteroid(gameState.asteroids)) then
            ...
            let position' = this.position + this.velocity * ((float32)dt.ElapsedGameTime.TotalMilliseconds) |> containInsideScreen
...
and Asteroid = {
    ...
    member this.Update(dt, gameState) =
        ...
        let position' = this.position + this.velocity |> containInsideScreen
